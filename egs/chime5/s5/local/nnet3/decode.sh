...
if [ $stage -le 6 ]; then
  ...
  steps/nnet3/decode.sh --nj $decode_num_jobs --cmd "$decode_cmd" $pass2_decode_opts \
     ...
     $graph ${segmented_data}_hires ${decode_dir}_stage2 || touch ${decode_dir}_stage2/.error
  [ -f ${decode_dir}_stage2/.error ] && echo "$0: Error decoding" && exit 1;
fi
exit 0
