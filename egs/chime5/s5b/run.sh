...
train_set=train_worn_Image_u400k
#train_set=train_worn_u400k
test_sets="dev_${enhancement}_dereverb_ref dev_worn" #"dev_worn dev_addition_dereverb_ref"
...
if [ $stage -le 5 ]; then
  echo "Stage 5"
  # Data augmentation with Image method and 3D simulation
  for datatype in Image 3D; do
    local/prepare_data.sh --mictype aug --datatype $datatype \
      ${audio_dir}/train ${json_dir}/train data/train_aug_${datatype}
  done
...
#  utils/subset_data_dir.sh data/train_uall 400000 data/train_u400k
  for datatype in Image 3D; do
    utils/subset_data_dir.sh data/train_aug_${datatype} 298912 data/train_aug300k_${datatype}
    utils/subset_data_dir.sh data/train_aug_${datatype} 298912 data/train_aug300k_${datatype}
  done
#  utils/combine_data.sh data/${train_set} data/train_worn_stereo data/train_u400k data/train_worn_rvb
#  utils/combine_data.sh data/${train_set} data/train_worn_stereo data/train_u400k
  utils/combine_data.sh data/train_worn_3D_u400k data/train_worn_stereo data/train_u400k data/train_aug300k_3D
  utils/combine_data.sh data/train_worn_Image_u400k data/train_worn_stereo data/train_u400k data/train_aug300k_Image
...
if [ $stage -le 18 ]; then
  ...
  local/chain/tuning/run_tdnn_1b.sh --nj ${nj} \
    ...
    --test-sets "$test_sets" \
    --gmm tri3_cleaned --nnet3-affix _${train_set}_cleaned #_rvb
fi
echo "Stage 18" && exit 1
if [ $stage -le 19 ]; then
  echo "Stage 19"
